...
export const metadata: Metadata = {
  ...
  description:
    'Compelling description with keywords (150-160 chars; Google typically displays this range)',
...
export default async function Image({
  ...
  return new ImageResponse(
    <div
      style={{ display: 'flex', fontSize: 64, width: '100%', height: '100%' }}
    >
      {post.title}
    </div>,
    { ...size },
...
- No `cookies()`/`headers()`/`searchParams` inside a plain `"use cache"` scope — good for SEO, since indexable content should be request-agnostic. (`"use cache: private"` _does_ allow them, but is never prerendered, so it never lands in the static SEO shell.)
